...
- 支持**粘贴视频直链**：先尝试前端直接下载，源站不允许跨域时自动改走同源代理
  `/fetch`（部署环境是 `functions/fetch.js` 这个 Cloudflare Pages Function，本地开发是
  `serve.mjs` 里的同款实现）。注意：链接功能需要部署版或本地服务器，双击打开的
  `index.html` 没有代理可用；代理是公开端点，建议在 Cloudflare 上给 `/fetch` 加
  WAF 限流规则防滥用
